<?php         
  ...
  */
  ...
  {
    $sql = sprintf(
      "INSERT INTO MARCELO_VENDAS (CNPJ, EMPRESA, EMISSAO, COD_OPERADOR, OPERADOR, INICIO, CUPONS, TOTAL) VALUES('%s', '%s', '%s', %s, '%s', '%s', %s, %s)", 
  ...
  {
    $sql = "SELECT * FROM MARCELO_VENDAS";
  ...
  {
    $sql = sprintf("DELETE FROM MARCELO_VENDAS WHERE ID = %s", $id);
  ...
  else if($method == "qtdevenda.add")
  {
    $sql = sprintf(
      "INSERT INTO MARCELO_QTDE_VENDAS (CNPJ, EMPRESA, EMISSAO, ID_MENOR, ID_MAIOR, CONTAGEM, DIFERENCA) VALUES('%s', '%s', '%s', %s, %s, %s, %s)", 
      $cnpj, $empresa, $emissao, $menor, $maior, $contagem, $diferenca
    );
  ...
  else if($method == "qtdevenda.get")
  {
  }
  else if($method == "qtdevenda.delete")
  {
  }
  db_close();
?>