<?php
  ...
  if(empty($method))
  {
    echo "Informe um dos methods: send_image, list_image, list_date";
    exit;
  }
  ...
  if($method == "send_image")
  {
    ...
    $sql = sprintf(
      "INSERT INTO REG_REGISTRO(REG_DATA, REG_IMAGEM, REG_AUTENTICACAO) VALUES (now(),'%s','%s');", 
      $image, $authenticate
    );
  ...
  else if($method == "list_image")
  {
    $sql = sprintf("
            ...
            ORDER BY REG_CODIGO LIMIT %s", $code, $lastdate, $authenticate, $count);
    $result = db_query($sql);
