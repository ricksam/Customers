...
{
  public partial class frmEstorno : lib.Visual.Models.frmEdit
  {
    public frmEstorno()
    {
      InitializeComponent();
    ...
    private void Carregar()
    {
      ...
      Grid.AddColumn(new FieldColumn("Data Pgto.", "FIN_DTPGTO", enmFieldType.Date));
      ...
      Grid.AddItems(dsFin.GetList_FromBaixado());
    }
    ...
    {
      if (lib.Visual.Msg.Question("Tem certeza que deseja estornar os títulos marcados?"))
      {
        ...
        for (int i = 0; i < lst.Length; i++)
        {
          if (lst[i].Sel)
          {
            lst[i].FIN_DTPGTO = DateTime.MinValue;
            dsFin.Save(lst[i]);
          }
        }
        base.OnConfirm();
    ...
    private void frmEstorno_Activated(object sender, EventArgs e)
    {
