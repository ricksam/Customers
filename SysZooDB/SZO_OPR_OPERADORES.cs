...
namespace SysZoo
{
  ...
  public class dsSZO_OPR_OPERADORES : DefaultDataSource<SZO_OPR_OPERADORES>
  {
    ...
    public SZO_OPR_OPERADORES Get_FromMD5(string OPR_MD5, System.Data.Common.DbTransaction transaction = null)
    {
      return Get("SELECT * FROM SZO_OPR_OPERADORES WHERE OPR_MD5 = " + DbQuoted(OPR_MD5), transaction);
    }

    public SZO_OPR_OPERADORES Get_FromSenha(string OPR_SENHA)
    {
      return Get("SELECT * FROM SZO_OPR_OPERADORES WHERE (OPR_INATIVO = 0 OR OPR_INATIVO IS NULL) AND OPR_SENHA = " + DbQuoted(lib.Class.EncryptionDeprecated.GetMD5(OPR_SENHA)));
    }
    ...
    public void Save(SZO_OPR_OPERADORES tab, System.Data.Common.DbTransaction transaction = null)
    {
      ...
      { tab.OPR_SENHA = lib.Class.EncryptionDeprecated.GetMD5(tab.OPR_SENHA); }

      if (tab.OPR_CODIGO == 0)
      {
        tab.OPR_TIMESTAMP = DateTime.UtcNow;
        if (string.IsNullOrEmpty(tab.OPR_MD5))
        { tab.OPR_MD5 = lib.Class.EncryptionDeprecated.GetMD5(Guid.NewGuid().ToString()); }
    ...
    public void InativaOperadores(System.Data.Common.DbTransaction transaction)
    {
        DbBase.DbExecute("UPDATE SZO_OPR_OPERADORES SET OPR_INATIVO = 1 WHERE OPR_SINCRONIZADO = 1", transaction);
    }
  }
}
